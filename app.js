...
function renderFlags() {
    ...
    // on récupère ceux avec le maximum de couleurs en commun
    var best_match = flags.filter(function (flag) {
        ...
        // si le nombre de couleurs trouvées est égale
        // au nombre de couleurs contenu par le drapeau
        // on a un maximum de couleurs en commun
        return count === colors.length;
    });

    // si on a moins de 3 pays trouvé, on continue
    // en cherchant les pays avec au moins 1 couleur en commun
    if (best_match.length < 3) {
        // d'abord on élimine ceux qui ont un max de couleur en commun
        var remaining = flags.filter(function(flag) {
            return best_match.indexOf(flag) === -1;
        });

        // on récupère maintenant ceux avec au moins 1 couleur en commun
        ...
        // on concatène ceux qui ont un maximum de couleurs
        // avec ceux qui ont au moins une couleur en commun
        best_match = best_match.concat(new_best_match);
    }

    // parmis les match, on prend les 3 premiers
